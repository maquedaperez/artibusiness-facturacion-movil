...
@Component({
  ...
  template: `
    <ion-header>
      <ion-toolbar>
        <ion-title>Producto / servicio de catálogo</ion-title>
    ...
    <ion-content class="ion-padding">
      <ion-searchbar
        placeholder="Buscar por nombre o referencia (mínimo 2 caracteres)"
        [(ngModel)]="query"
      ...
      <ion-list *ngIf="estado === 'ok'">
        <ion-item *ngFor="let p of resultados" button (click)="seleccionar(p)">
          <ion-label>
            <h2>{{ p.nombre }}</h2>
            <p>{{ p.referencia ? p.referencia + ' · ' : '' }}{{ formatEuros(p.precioUnitario) }} · IVA {{ p.ivaPct }}%</p>
          </ion-label>
        </ion-item>
      </ion-list>
    </ion-content>
